...
| `quickSearch`  | `"new york" miami`                                                                                | splits on whitespace, keeps double-quoted phrases as one term, and matches when any term is a substring of any field (`id`, `companyName`, `origin`, `destination`, `weight`, `equipmentType`, `date`, `price`, `distance`, `status`). Leading and trailing whitespace is ignored. Alias: `q` (ignored if trimmed `quickSearch` is non-empty) |
...
`GET /loads`: whitespace-delimited terms use OR semantics, so a load matches
when any term is a substring of any field. Double quotes group a phrase into
one term (for example, `"new york" miami` searches for `new york` or `miami`),
and `\"` represents a literal quote inside a phrase. Leading and trailing
whitespace is ignored. A non-empty trimmed `quickSearch` wins when both aliases
are present; otherwise the trimmed `q` value is used. Other filtering, sorting,
and pagination parameters do not apply.
