...
| `quickSearch`  | `chicago miami`                                                                                   | splits on whitespace and matches when any term is a substring of any field (`id`, `companyName`, `origin`, `destination`, `weight`, `equipmentType`, `date`, `price`, `distance`, `status`), mirroring the client's AG Grid quick filter. Leading and trailing whitespace is ignored. Alias: `q` (ignored if trimmed `quickSearch` is non-empty) |
...
when any term is a substring of any field. Leading and trailing whitespace is
ignored. A non-empty trimmed `quickSearch` wins when both aliases are present;
otherwise the trimmed `q` value is used. Other filtering, sorting, and
pagination parameters do not apply.
