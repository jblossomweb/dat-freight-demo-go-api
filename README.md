...
| `quickSearch`  | `chicago`                                                                                         | matches every field (`id`, `companyName`, `origin`, `destination`, `weight`, `equipmentType`, `date`, `price`, `distance`, `status`), mirroring the client's AG Grid quick filter. Leading and trailing whitespace is ignored; internal whitespace is preserved. Alias: `q` (ignored if trimmed `quickSearch` is non-empty)    |
...
`GET /loads`. Leading and trailing whitespace is ignored while internal
whitespace is preserved. A non-empty trimmed `quickSearch` wins when both
aliases are present; otherwise the trimmed `q` value is used. Other filtering,
